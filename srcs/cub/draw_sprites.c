...
/*   Updated: 2020/03/12 18:00:26 by akerdeka         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_sprites(t_cub_struct *c)
{
	...
	while (i < c->Numsprite)
	{
		...
		c->transform.x = c->invdet * (c->dir.y * c->spritex - c->dir.x * c->spritey);
		c->transform.y = c->invdet * (-c->plane.y * c->spritex + c->plane.x * c->spritey);
		c->sprite_scrn_x = (int)((c->res_win[1] / 2) * (1 + c->transform.x / c->transform.y));
		...
		{
			c->tex_x = (int)((256 * (c->stripe - (-c->sprite_width / 2 + c->sprite_scrn_x)) * c->text[4].width / c->sprite_width) / 256);
			if (c->transform.y > 0 && c->stripe < c->res_win[1] && c->transform.y < c->zbuffer[c->stripe])
			{
				y = c->sprite_start.y;
				while (y < c->sprite_end.y)
				{
					d = y * 256 - c->res_win[2] * 128 + c->sprite_height * 128;
					c->tex_y = ((d * c->text[4].width) / c->sprite_height) / 256;
					c->color->color = c->text[4].img_data[c->tex_y * c->text[4].width + c->tex_x];
					if ((c->color->color & 0xffffff) != 0)
						c->screen->img_data[y * c->res_win[1] + c->stripe] = c->color->color;
					y++;
				}
			}
			c->stripe++;
